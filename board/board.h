...
/* @Brief Board PCA9957 LED configuration */
#define BOARD_LED_SPI_BASEADDR              (LPSPI1)
#define BOARD_LED_SPI_CLOCK_SOURCE_SELECT 	(1U)
#define BOARD_LED_SPI_CLOCK_SOURCE_DIVIDER 	(7U)
#define BOARD_LED_SPI_CLK_FREQ 				(CLOCK_GetFreq(kCLOCK_Usb1PllPfd0Clk) / (BOARD_LED_SPI_CLOCK_SOURCE_DIVIDER + 1U))
#define BOARD_LED_SPI_PCS_FOR_TRANSFER 		(kLPSPI_MasterPcs0)
#define BOARD_LED_SPI_nRST_ON  (0U)
#define BOARD_LED_SPI_nRST_OFF (1U)
#define BOARD_LED_SPI_nRST_GPIO GPIO1
#define BOARD_LED_SPI_nRST_GPIO_PIN (23U)
...
#if defined(SDK_SPI_BASED_COMPONENT_USED) && SDK_SPI_BASED_COMPONENT_USED
void BOARD_PCA9957_SPI_Init(void);
status_t BOARD_PCA9957_LPSPI_XFER(
		uint8_t deviceCS,
		uint8_t *txBuff,
		uint8_t *rxBuff,
		uint8_t xferBuffSize);
#endif
...
status_t BOARD_LPI2C_Receive(LPI2C_Type *base,
                             ...
                             uint8_t *rxBuff,
                             uint8_t rxBuffSize);
void BOARD_TSL2591_I2C_Init(void);
status_t BOARD_TSL2591_I2C_Send(uint8_t deviceAddress, uint32_t subAddress, uint8_t subaddressSize, uint32_t txBuff);
status_t BOARD_TSL2591_I2C_Send_Clear(uint8_t deviceAddress, uint32_t subAddress, uint8_t subaddressSize);
status_t BOARD_TSL2591_I2C_Receive(
